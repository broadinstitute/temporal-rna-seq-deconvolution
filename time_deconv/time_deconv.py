...
class TimeRegularizedDeconvolution:
    ...
    def model(
        ...
        if self.basis_functions == 'polynomial':
            ...
            c_kl = legendre_coefficient_mat(self.polynomial_degree, dtype = dtype)[1:,].to(device) # drop constant term
...
def calculate_prediction_error(sim_res, pseudo_time_reg_deconv_sim, n_intervals = 10):
    ...
    L2_error = (ground_truth_proportions_cm - ret_vals['norm_comp_t']).pow(2).sum([0,1]).sqrt()
