...
def hypercluster_anndata(
    ...
    cell_types_identical = list( k for k in ct_counter if ct_counter[k] <= min_cells_recluster )
    cell_types_recluster = list( k for k in ct_counter if ct_counter[k] > min_cells_recluster )
    ...
    for ct in cell_types_recluster:
        ...
        original_cell_names = list(ann_data_working.obs.loc[ann_data_working.obs[original_clustering_name] == ct].index)
        ...
        for nct in new_cluster_names:
            cells_in_new_cluster = list(ann_data_working.obs[ann_data_working.obs[restricted_name] == nct].index)
    ...
    for ct in cell_types_identical:
        ...
        cells_in_celltype = list(ann_data_working.obs[original_clustering_name].index[sel])
        new_cell_mapping[ct] = cells_in_celltype
        
        cluster_map[ct] = ct
    
    if verbose:
        print(f'Keeping {len(new_clusters_keep)}/{len(new_clusters_keep) + len(discarded_clusters)} clusters')
    ...
    new_cluster_df = pd.DataFrame({cell_id_column_name: cn_new, hypercluster_columns_name: cluster_new}).set_index(cell_id_column_name)
