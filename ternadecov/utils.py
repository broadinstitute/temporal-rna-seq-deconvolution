...
def melt_tensor_to_pandas(input_tensor: torch.Tensor, dimnames, *dimlabels) -> pd.DataFrame:
    ...
    for n in range(input_tensor_dim):
        ...
        output_label_idx.append(
            torch.tile(
                torch.arange(
                    len(dimlabels[n])).repeat_interleave(e), (a, )
            )
        )
    ...
    for j in range(len(output_label_idx)):
        output[dimnames[j]] = pd.Categorical(output_label_idx[j].numpy()).rename_categories(dimlabels[j])
